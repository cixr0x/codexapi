...
After building the checked-in revision on the production VM, a root operator can run `sudo npm run --silent verify:isolation` (or `sudo node dist/src/verifyIsolation.js`). It calls only `http://127.0.0.1:3001`, creates random disposable markers in fixed protected roots, and prints exactly `{"status":"ok","isolation":"verified"}` when the hostile-access and cancellation-cleanup probes pass.
