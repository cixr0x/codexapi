...
After building the checked-in revision on the production VM, a root operator can run `sudo npm run --silent verify:isolation` (or `sudo node dist/src/verifyIsolation.js`). It calls only `http://127.0.0.1:3001`, creates a random root-owned `0711` canary directory and a `0400` `codexapi` marker inside each fixed protected root, and prints exactly `{"status":"ok","isolation":"verified"}` when the hostile-access and cancellation-cleanup probes pass. The directory permits traversal without allowing the service to replace, link, rename, or list the marker; cleanup attests both directory and marker identities before removal. Where a protected root's parent is not traversable by the service, the systemd path denial is the acceptance layer and the canary directory remains as permissive as this ownership boundary safely permits.
